...
$(document).ready(function(){
  ...
  $(document).prop('title', 'Clientes - Satvicos');
});
...
$("#FRM_INSERT_CLIENTE").submit(function (e) {
    ...
    if (!formData_rec.get('cliente_razsoc') || !formData_rec.get('cliente_ruc')) {
        $.Notification.notify("error", "bottom-right", "Datos incompletos", "Por favor, completa todos los campos requeridos.");
        return;
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log("Respuesta del servidor:", data); // Inspecciona la respuesta aquí
            ...
            if (data == "ERROR") {
                ...
                $.Notification.notify("error", "bottom-right", "Respuesta inesperada", "El servidor devolvió una respuesta desconocida.");
                Swal.close();
            }
        },
        error: function (xhr, status, error) {
            console.error("Error en la solicitud:", status, error); // Muestra el error en consola
            Swal.close();
            $.Notification.notify("error", "bottom-right", "Error de red", "No se pudo procesar la solicitud. Por favor, intenta nuevamente.");
        }
    });
});
